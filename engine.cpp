...
#include <time.h>      // time
#include <iostream>    // various ... needs to be removed.
...
int main(int argc, char *argv[]) {
	...
	std::cout << "h_res: " << h_res << " v_res: " << v_res << std::endl;
	...
	if (input_filename == NULL) {
		std::cout << "Please enter an input filename" << std::endl;
		return 0;
...
//			  NULL));
		...
		if (parse_input(input,&cam_pos,object_list,light_list) != 0) {
			std::cout << "error in input file " << input_filename << ". Aborting..\n";
			return 0;
		}
	}
...
//			real_j = int(v_res - j);
	...
	if (flags_time) 
		std::cout << "Render time: " << int(end-begin)/3600 << "h " << int((end-begin)/60)%60 << "m " << (end-begin)%60 << "s" << std::endl;
...
void print_lightlist(light_node *l) {
	...
	while (l != NULL) {
		...
		std::cout << "  pos   : "; print_vector(l->l.pos);
		std::cout << "  color : " << (int)l->l.hue.r << " " << (int)l->l.hue.g << " " << (int)l->l.hue.b << std::endl;
...
int parse_args(int argc, char *argv[]) {
	if (argc == 1) {
		std::cout << "Usage : render.exe [options]\n"
			<< "Options: \n"
...
//	   make this a lot cleaner...
			...
			while (!feof(input)) {
				...
				if ((ptr = strstr(line,"CameraPos")) != NULL) {
					...
					if (sscanf(ptr,"%f %f %f",&(cam->origin.x),&(cam->origin.y),&(cam->origin.z)) != 3) {
						std::cout << "Error in CameraPos declaration.\n";
						return 1;
					}
					cam_pos = true;
				}
					...
					if (sscanf(ptr,"%f %f",&theta,&phi) != 2) {
						std::cout << "Error in CameraView declaration.\n";
						return 1;
					}
		...
		if ((ptr = strstr(line,"Light")) != NULL) {
			...
			if ((ptr = strstr(line,"Ambient")) != NULL) {
				...
				if (sscanf(ptr,"%d %d %d",&(ambient_light.r),&(ambient_light.g),&(ambient_light.b)) != 3) {
					std::cout << "Error in Light::Ambient declaration.\n";
					return 1;
				}
			}
			else {
				...
				if ((ptr = strstr(line,"Point")) != NULL) {
					...
					if ((ptr = strstr(line,"Pos")) != NULL) {
						...
						if (sscanf(ptr,"%f %f %f",&(light_temp->l.pos.x),&(light_temp->l.pos.y),&(light_temp->l.pos.z)) != 3) {
							std::cout << "Error in Light::Point::Pos declaration, line " << line_num << std::endl;
							return 1;
						}
					}
					else {
					...
					if ((ptr = strstr(line,"Hue")) != NULL) {
						...
						if (sscanf(ptr,"%d %d %d",&(light_temp->l.hue.r),&(light_temp->l.hue.g),&(light_temp->l.hue.b)) != 3) {
							std::cout << "Error in Light::Point::Hue declaration.\n";
							return 1;
						}
					}
					else {
				...
				if ((ptr = strstr(line,"Tri")) != NULL) {
					for (num_points = 0; num_points < 3; num_points++) {
						...
						if ((ptr = strstr(line,"Vertex")) != NULL) {
							...
							if (sscanf(ptr,"%f %f %f",&(point[num_points].x),&(point[num_points].y),&(point[num_points].z)) != 3) {
								std::cout << "Error in Tri::Vertex declaration, line " << line_num << std::endl;
								return 1;
							}
						}
						else {
					...
					if ((ptr = strstr(line,"Color")) != NULL) {
						...
						if (sscanf(ptr,"%d %d %d %d %d",&(obj_color.r),&(obj_color.g),&(obj_color.b),&(obj_color.a),&(obj_color.ref)) != 5) {
							std::cout << "Error in Tri::Color declaration, line " << line_num << std::endl;
							return 1;
						}
					}
					else {
					...
					if ((ptr = strstr(line,"Origin")) != NULL) {
						...
						if (sscanf(ptr,"%f %f %f",&(sphere_origin.x),&(sphere_origin.y),&(sphere_origin.z)) != 3) {
							std::cout << "Error in Sphere::Origin declaration, line " << line_num << std::endl;
							return 1;
						}
					}
					else {
					...
					if ((ptr = strstr(line,"Radius")) != NULL) {
						...
						if (sscanf(ptr,"%f",&sphere_radius) != 1) {
							std::cout << "Error in Sphere::Radius declaration, line " << line_num << std::endl;
							return 1;
						}
					}
					else {
					...
					if ((ptr = strstr(line,"Color")) != NULL) {
						ptr += strlen("Color");
						if (sscanf(ptr,"%d %d %d %d %d",&(obj_color.r),&(obj_color.g),&(obj_color.b),&(obj_color.a),&(obj_color.ref)) != 5) {
							std::cout << "Error in Sphere::Color declaration, line " << line_num << std::endl;
							return 1;
						}
					}
					else {
	...
	std::cout << "Parsed " << line_num << " line(s) successfully." << std::endl;
